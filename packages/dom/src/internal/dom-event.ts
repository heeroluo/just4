...
const fixEventType = (function() {
  ...
  // 记录需要添加前缀的事件名
  const vendorEventTypes: { [key: string]: string } = Object.create(null);
  // 用于检查是否支持事件的小白鼠元素
...
const listenerManager = (function() {
  ...
  const listenerSpace = createDataSpace({
    // 克隆节点时要把事件监听也克隆过去
    onClone: function(target): void {
      this.keys(target).forEach(function(eventType) {
        (<EventTarget>target).addEventListener(eventType, dispatch, false);
      });
    }
  });

  // 找到事件监听的委托元素
  function findDelegator(evtWrap: EventWrap, delegator: string): Node | undefined {
    let node: Node | null = <Node>evtWrap.target;
  ...
  function dispatch(this: EventTarget, e: Event): void {
    // 处理事件参数兼容性
    const evtWrap = new EventWrap(e, this);
    const eventType = evtWrap.type;

    const listeners = <IEventListener[]>(
      listenerSpace.getData(evtWrap.currentTarget, eventType)
    );
    if (listeners) {
      // 遍历回调函数
      listeners.forEach(function(listener) {
        let trueThis: EventTarget | undefined;
        ...
        if (result === false) { evtWrap.preventDefault(); }
      });
    }
  }
  ...
  return {
    // 添加监听器
    add(target: EventTarget, type: string, listener: IEventListener) {
      let listeners = <IEventListener[]>listenerSpace.getData(target, type);
      if (!listeners) {
        // 创建事件监听队列
        listeners = [];
...
// 支持事件接口时执行执行指定函数
function ifSupportsEvent(
  node: DOMWrapMember | ArrayLike<DOMWrapMember>,
  fn: (node: EventTarget) => void
): void {
  if (isArrayLike(node)) {
    ...
    if (node != null && typeof (<any>node).addEventListener === 'function') {
      fn(<EventTarget>node);
    }
  }
}
...
export function onEvent(
  ...
  if (!typeArr.length || !handler) { return; }
...
// 判断是否可以使用事件方法方案
const shouldCallEventMethod: { [key: string]: (node: Node) => boolean } = {
  focus(node: Node): boolean {
    // 可聚焦的元素，才能通过 .focus() 触发 focus 的监听函数
    return /^(?:input|select|textarea|button)$/i.test(node.nodeName);
  },

  blur(node: Node): boolean {
    // 当前聚焦元素，才能通过 .blur() 触发 blur 的监听函数
    return node.ownerDocument != null && node.ownerDocument.activeElement === node;
  },

  reset(node: Node): boolean { return node.nodeName === 'FORM'; },

  submit(node: Node): boolean { return node.nodeName === 'FORM'; },
...
export function triggerEvent(
  nodes: ArrayLike<DOMWrapMember>, type: string
): void {
  ...
  ifSupportsEvent(nodes, function(node) {
    if (
      isNode(node) &&
      typeof (<any>node)[type] === 'function' &&
      hasOwnProp(shouldCallEventMethod, type) &&
      shouldCallEventMethod[type](<Node>node)
    ) {
      (<any>node)[type]();
      return;
    }
